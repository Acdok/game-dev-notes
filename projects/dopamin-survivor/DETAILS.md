...
8. [바이브코딩 개발 방식과 검증 범위](#8-바이브코딩-개발-방식과-검증-범위)
9. [소스 안내](#9-소스-안내)
...
제공 녹화 약 00:47. 캐릭터에서 발사된 붉은 레이저가 적 사이로 휘어지는 장면입니다. 아래 시너지 설명은 코드 기준이며, 이 한 장면이 모든 조합을 증명하는 것은 아닙니다.

| 녹화 구간 | 화면에서 확인한 내용 |
|---|---|
| 초반 | 이동, 일반 탄환 자동 발사, 적 추적, 경험치 오브젝트 |
| 약 00:08 | 레벨업 3지선다 보상 UI |
| 약 00:23 | 보유 아이템과 과충전 사격 세트 핵심 단계 |
| 약 00:38 이후 | 붉은 혈사포의 직선·곡선 발사 장면 |

시간은 게임 HUD가 아니라 **제공된 녹화 시작 기준의 대략적인 위치**입니다. 녹화 프레임레이트를 게임 성능 수치로 사용하지 않았습니다.
...
`WeaponController`는 공격 간격을 채운 뒤 유효한 목표가 있을 때 발사합니다. 목표가 없을 때도 준비 상태를 유지하므로, 적이 생기면 준비된 공격을 사용할 수 있습니다. 혈사포 역시 이 자동 발사 흐름에 연결되며, 버튼을 계속 누르는 수동 차징 방식은 아닙니다.
...
보스 생성과 사망 시 보상 상자를 만드는 코드도 있습니다. 보스 상자는 일반 레벨업과 다른 티어 가중치를 사용합니다. 다만 제공된 영상만으로 보스전·상자 보상 전체 흐름까지 검증한 것은 아닙니다.
...
| 혈사포 + 같은 과충전 세트의 다른 아이템 | 부모 혈사포에 유도 적용 | 서로 다른 2종을 보유한 ‘예열’ 단계부터 적용 |
...
성장 기능의 연결을 시작 단계에서 모으고, 전투·인벤토리·보상·UI의 책임은 별도 컴포넌트로 나눴습니다. 이는 현재 프로토타입의 구성 방식이며, 모든 씬과 빌드 환경에서의 안정성 검증 완료를 뜻하지 않습니다.

## 8. 바이브코딩 개발 방식과 검증 범위

이 프로젝트는 **Unity와 AI 보조 개발(바이브코딩)**을 활용해 제작했습니다. 저장된 기획 구조, 작업 명세, 체크리스트를 통해 성장 기능과 공격 조합을 작업 단위로 나눈 흔적을 확인할 수 있습니다.

AI 활용은 프로젝트의 개발 방식으로 소개하되, 특정 모델이나 사용하지 않았다고 확인된 도구를 임의로 나열하지 않습니다. 생성된 코드가 있다는 사실과 실행 검증을 마쳤다는 주장은 구분합니다.

### 현재 근거가 있는 범위

- **실행 녹화:** 일반 전투, 경험치·보상 화면, 보유 아이템·세트 UI, 혈사포 직선·곡선 연출.
- **정적 코드 확인:** 다중 발사 우선순위, 세트 조건, 부모·자식 유도 전달, 재분열 제한, 보상 추첨, 보스 상자 코드.
- **저장된 작업 기록:** 경험치·아이템·세트 구현 항목과 수동 확인 항목이 구분되어 있습니다. 수동 검증 체크가 비어 있는 항목을 통과 결과로 바꾸어 설명하지 않았습니다.

### 테스트용 입력과 영상 해석

프로젝트에는 숫자키로 더블·트리플·쿼드샷·분열·유도·혈사포를 강제 전환하는 테스트 경로가 있습니다. 이 경로는 실제 인벤토리 획득과 별개라서 화면의 아이템 목록만으로 공격의 활성화 원인을 확정할 수 없습니다. 영상의 혈사포를 자연 드롭으로 획득한 장면이라고 단정하지 않았습니다.

### 추가 확인이 필요한 부분

- 모든 조합을 실제 아이템 획득 경로와 테스트 토글 경로에서 각각 확인하기.
- 다중 발사·분열이 겹쳤을 때 생성 수, 피해량, 자식의 재분열 방지를 확인하기.
- 보상 숫자키와 테스트 숫자키의 입력 구분, 연속 레벨업 후 시간 배율 복귀를 확인하기.
- 보스 상자 획득부터 보상 선택까지 실행 장면으로 보강하기.
- 적이 많을 때 대상 탐색과 레이저 판정 비용을 프로파일링하기.

현재 코드에는 런타임 객체 생성·파괴와 대상 검색이 있으므로 오브젝트 풀링 완료, 대규모 적 처리 성능 보장, 최적화 개선율 등의 성과를 주장하지 않습니다.
