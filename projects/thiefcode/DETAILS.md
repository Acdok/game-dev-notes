...
이 프로젝트에서 제가 설명하고싶은건 AI를 단순하게 사용했다가 아닙니다. 
AI가 사람의 요구를 실행 가능한 Task와 Checklist로 바꾸고, 에이전트가 수정할 모듈과 사용할 도구를 제한한 뒤, 에디터·테스트·패키지 결과로 통과 여부를 판정하는 개발 방식이 이 프로젝트의 핵심입니다.

### 기여와 소유권

| 구분 | 범위 |
|---|---|
사실 이 프로젝트의 설계 측면에서는 AI가 대부분을 차지하고있습니다.
저와 팀원은 아이디어를 선정했고, 그 아이디어를 토대로 AI를 사용해 기획부터 설계까지 한 후,
같의 의논하며 어떤부분을 고쳐야 할 지 md파일을 수정하고 실행했습니다.

그래서 기존 방식처럼 각자의 커다란 역할을 맡아 개발한 것이 아닙니다.

서로 GPT Plus를 쓰며 각자 맡은 Task가 먼저 완료되면
다음 Task를 진행하기도 하고, Task 진행도중 토큰을 다써서 다음 사람이 그대로 이어받기도 했습니다.
동시에 작업하며 충돌의 우려가 있었지만 AI에게 Task 작업 분리를 맡겨놓으니 충돌하지 않았습니다.

<img width="1708" height="589" alt="image" src="https://github.com/user-attachments/assets/defc84f9-01f0-4331-b9be-cb6f8e248739" />

<img width="1022" height="763" alt="image" src="https://github.com/user-attachments/assets/e49a89e6-be69-44a9-902f-bd0c2b93b56d" />

보다시피 각자의 작업을 A와 B로 나눠 충돌방지를 하고, Checklist들을 검증해나가며 동시에 작업하니 속도가 아주 빨랐습니다. (토큰에 따라 Task를 끝내고 A를 하고 B를 하기도 했습니다.)

물론 처음 Task들은 하네스 구성이 엉성했어서 제대로 결과물이 나오지 않았지만 수정을 거듭하며 프로젝트에 맞는
규칙들을 적용시키니 갈수록 오류가 적어졌습니다. 
